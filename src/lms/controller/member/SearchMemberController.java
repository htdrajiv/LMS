...
public class SearchMemberController implements Initializable {
	...
	public void searchMembers() {
		...
		LibraryMember mem = IDataAccess.searchMember(txtMemberId.getText());
	...
	public void goToEditMember() {
		...
		member = IDataAccess.searchMember(mem.getIdNumber());
	...
	public void initialize(URL location, ResourceBundle resources) {
		if (location.toString().contains("searchMembers.fxml")) {
			HashMap<String, LibraryMember> allMembers = IDataAccess
					.readMemberMap();
			if (allMembers != null) {
	...
	public void applySaveMember() {
		...
		if (allMember != null) {
			if (member == null) {
				...
				IDataAccess.saveNewMember(newMember);
			} else {
				// edit member
				...
				LibraryMember editedMember = new LibraryMember(
						txtFirstName.getText(), txtLastName.getText(),
						txtPhone.getText(), memberAddress, memberId);
				IDataAccess.saveNewMember(editedMember);
			}
